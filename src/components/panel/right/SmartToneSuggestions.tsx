...
export default function SmartToneSuggestions({
  ...
  const generateSuggestionPreviews = useCallback(
    async (nextSuggestions: SmartToneSuggestion[]) => {
      if (!selectedImage?.isReady || nextSuggestions.length === 0) return;
      ...
      try {
        for (const suggestion of nextSuggestions) {
          if (pathAtStart !== imagePathRef.current) break;
          ...
          try {
            ...
            console.error(`Failed to generate smart preset preview for ${suggestion.name}:`, error);
            setPreviewUrls((prev) => ({ ...prev, [suggestion.id]: null }));
          }
        }
      } finally {
        if (pathAtStart === imagePathRef.current) setIsGeneratingPreviews(false);
      }
    },
              ...
              >
                <div className="aspect-[4/3] bg-surface flex items-center justify-center overflow-hidden">
                  {previewUrl ? (
                    ...
                    <Loader2 size={18} className={clsx('text-text-secondary', isPreviewLoading && 'animate-spin')} />
                  )}
                </div>
                <div className="p-2">
